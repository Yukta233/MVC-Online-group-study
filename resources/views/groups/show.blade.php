...
<div class="workspace-main">
    ...
    <div class="workspace-tabs">
        ...
        <button class="tab-trigger" onclick="switchTab(event, 'scheduler')">
            <i class="fa-solid fa-calendar-days"></i> Meet Planner
        </button>
        <button class="tab-trigger" onclick="switchTab(event, 'video')">
            <i class="fa-solid fa-video"></i> Live Call
        </button>
        <button class="tab-trigger" onclick="switchTab(event, 'code')">
            <i class="fa-solid fa-code"></i> Code Arena
        </button>
        <button class="tab-trigger" onclick="switchTab(event, 'whiteboard')">
            <i class="fa-solid fa-chalkboard"></i> Whiteboard
        </button>
    ...
    <div class="tab-content">
        ...
        <!-- PANEL 5: Session Planner -->
        <div id="panel-scheduler" class="tab-panel">
            <div class="sessions-wrapper">
                <div class="drive-header">
                    <h3 style="font-size: 1.2rem; font-weight: 600;"><i class="fa-solid fa-calendar-days" style="color: var(--accent-indigo);"></i> Group Meetings Calendar</h3>
                    <button class="btn btn-primary" onclick="openModal('sessionModal')" style="width: auto; padding: 0.5rem 1.25rem;">
                        <i class="fa-solid fa-plus"></i> Schedule Meeting
                    </button>
                </div>

                @if($group->studySessions->isEmpty())
                    <div class="glass-panel" style="padding: 4rem; text-align: center; color: var(--text-secondary);">
                        <i class="fa-solid fa-video" style="font-size: 2.5rem; color: var(--text-muted); margin-bottom: 1rem; display: block;"></i>
                        <p style="font-weight: 500;">No live study sessions scheduled yet.</p>
                        <p style="font-size: 0.82rem; color: var(--text-muted); margin-top: 0.25rem;">Coordinate dates and publish study meeting invitations above!</p>
                    </div>
                @else
                    <div class="sessions-grid">
                        @foreach($group->studySessions as $session)
                            <div class="glass-panel glass-card session-card">
                                <div>
                                    ...
                                    <h4>{{ $session->title }}</h4>
                                    <p>{{ $session->description ?: 'No meeting goals specified.' }}</p>
                                </div>
                                <div style="border-top: 1px solid var(--border-color); padding-top: 0.75rem;">
                                    <div class="session-meta-info">
                                        <span><i class="fa-solid fa-hourglass-half"></i> {{ $session->duration_minutes }} minutes</span>
                                    </div>
                                    @if($session->meeting_link)
                                        <a href="{{ $session->meeting_link }}" target="_blank" class="session-join-btn">
                                            <i class="fa-solid fa-video"></i> Launch Google Meet
                                        </a>
                                    @else
                                        <button class="session-join-btn" disabled style="opacity: 0.5; cursor: not-allowed;">
                                            <i class="fa-solid fa-circle-info"></i> In-Person/No Link
                                        </button>
                                    @endif
                                </div>
                            </div>
        ...
        <!-- PANEL 7: Monaco Code Arena -->
        <div id="panel-code" class="tab-panel">
            <div class="glass-panel" style="padding: 1.5rem; margin-bottom: 1.5rem; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem; border: 1px solid var(--border-color);">
                <div>
                    <h3 style="font-size: 1.2rem; font-weight: 700; color: var(--accent-indigo);"><i class="fa-solid fa-code"></i> Monaco Code Arena</h3>
                    <p style="color: var(--text-secondary); font-size: 0.82rem; margin-top: 0.25rem;">Write, compile, and run multiple language scripts in an integrated sandboxed IDE.</p>
                </div>
                
                <div style="display: flex; gap: 0.75rem; align-items: center;">
                    <select id="editorLanguage" class="form-control" onchange="changeEditorLanguage()" style="padding: 0.45rem 1rem; font-size: 0.8rem; background: rgba(10, 11, 16, 0.9); border: 1px solid var(--border-color); color: var(--text-primary); border-radius: 6px; width: auto; height: auto;">
                    ...
                    <select id="editorTheme" class="form-control" onchange="changeEditorTheme()" style="padding: 0.45rem 1rem; font-size: 0.8rem; background: rgba(10, 11, 16, 0.9); border: 1px solid var(--border-color); color: var(--text-primary); border-radius: 6px; width: auto; height: auto;">
                        <option value="vs-dark">Dark Theme</option>
                        <option value="vs">Light Theme</option>
                        <option value="hc-black">High Contrast</option>
                    </select>
                    
                    <button class="btn btn-primary" onclick="runSandboxCode()" style="width: auto; padding: 0.5rem 1.5rem; border-radius: 6px; font-size: 0.8rem; font-weight: 600;">
                        <i class="fa-solid fa-play"></i> Run Code
                    </button>
                </div>
            </div>
            
            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 1.5rem; height: 500px; align-items: stretch; margin-bottom: 1.5rem;">
                <!-- Editor container -->
                <div class="glass-panel" style="padding: 0.5rem; border-radius: 12px; overflow: hidden; border: 1px solid var(--border-color); position: relative;">
                    <div id="monaco-editor-container" style="height: 100%; width: 100%; min-height: 480px; border-radius: 8px; overflow: hidden;"></div>
                </div>
                
                <!-- Terminal Output -->
                <div class="glass-panel" style="padding: 1.25rem; border-radius: 12px; display: flex; flex-direction: column; justify-content: space-between; border: 1px solid var(--border-color); background: rgba(10, 11, 16, 0.65);">
                    <div style="height: 100%; display: flex; flex-direction: column;">
                        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--border-color); padding-bottom: 0.5rem; margin-bottom: 0.75rem;">
                            <span style="font-family: monospace; font-size: 0.78rem; font-weight: 700; color: var(--text-muted); text-transform: uppercase;">
                                <i class="fa-solid fa-terminal"></i> Output Terminal
                            </span>
                            <button onclick="clearTerminalOutput()" class="resource-delete-btn" style="color: var(--text-muted); font-size: 0.72rem;" title="Clear Output">
                                <i class="fa-solid fa-trash-can"></i> Clear
                            </button>
                        </div>
                        
                        <div id="terminal-output" style="flex-grow: 1; overflow-y: auto; font-family: 'Courier New', Courier, monospace; font-size: 0.82rem; color: #10B981; line-height: 1.4; word-break: break-all; white-space: pre-wrap; padding: 0.75rem; background: rgba(0, 0, 0, 0.5); border-radius: 6px; border: 1px solid rgba(255, 255, 255, 0.05); min-height: 380px;">CollabSphere-IDE-Core v1.0.0 init...
Select a language, write your code, and click "Run Code" to view output.</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <!-- PANEL 8: Shared Whiteboard (Fabric.js Canvas) -->
        <div id="panel-whiteboard" class="tab-panel">
            <div class="glass-panel" style="padding: 1.5rem; margin-bottom: 1.5rem; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem; border: 1px solid var(--border-color);">
                <div>
                    <h3 style="font-size: 1.2rem; font-weight: 700; color: var(--accent-violet);"><i class="fa-solid fa-chalkboard"></i> Collaborative Whiteboard</h3>
                    <p style="color: var(--text-secondary); font-size: 0.82rem; margin-top: 0.25rem;">Draw diagrams, add shapes, write formulas, and save drafts directly to the Resources drive.</p>
                </div>
                
                <div style="display: flex; gap: 0.75rem; align-items: center; flex-wrap: wrap;">
                    <!-- Tool Selectors -->
                    <div class="glass-tool-group" style="display: flex; background: rgba(0,0,0,0.3); border-radius: 8px; padding: 0.25rem; border: 1px solid var(--border-color);">
                        <button class="btn btn-tool active" id="tool-pencil" onclick="setWhiteboardTool('pencil')" title="Pencil Brush" style="background: none; border: none; padding: 0.4rem 0.75rem; color: var(--text-primary); border-radius: 6px; cursor: pointer; transition: all 0.2s;"><i class="fa-solid fa-pencil"></i></button>
                        <button class="btn btn-tool" id="tool-select" onclick="setWhiteboardTool('select')" title="Selection Mode (Move / Resize)" style="background: none; border: none; padding: 0.4rem 0.75rem; color: var(--text-primary); border-radius: 6px; cursor: pointer; transition: all 0.2s;"><i class="fa-solid fa-arrow-pointer"></i></button>
                        <button class="btn btn-tool" id="tool-rect" onclick="addWhiteboardShape('rect')" title="Add Rectangle" style="background: none; border: none; padding: 0.4rem 0.75rem; color: var(--text-primary); border-radius: 6px; cursor: pointer; transition: all 0.2s;"><i class="fa-regular fa-square"></i></button>
                        <button class="btn btn-tool" id="tool-circle" onclick="addWhiteboardShape('circle')" title="Add Circle" style="background: none; border: none; padding: 0.4rem 0.75rem; color: var(--text-primary); border-radius: 6px; cursor: pointer; transition: all 0.2s;"><i class="fa-regular fa-circle"></i></button>
                        <button class="btn btn-tool" id="tool-text" onclick="addWhiteboardShape('text')" title="Add Text" style="background: none; border: none; padding: 0.4rem 0.75rem; color: var(--text-primary); border-radius: 6px; cursor: pointer; transition: all 0.2s;"><i class="fa-solid fa-font"></i></button>
                        <button class="btn btn-tool" id="tool-eraser" onclick="setWhiteboardTool('eraser')" title="Eraser Brush" style="background: none; border: none; padding: 0.4rem 0.75rem; color: var(--text-primary); border-radius: 6px; cursor: pointer; transition: all 0.2s;"><i class="fa-solid fa-eraser"></i></button>
                    </div>

                    <!-- Colors -->
                    <div style="display: flex; gap: 0.35rem; align-items: center; background: rgba(0,0,0,0.3); border-radius: 8px; padding: 0.35rem 0.6rem; border: 1px solid var(--border-color);">
                        <span style="font-size: 0.75rem; color: var(--text-muted); margin-right: 0.25rem;">Color:</span>
                        <div class="color-dot active-dot" onclick="setWhiteboardColor('#8b5cf6', this)" style="background: #8b5cf6; width: 16px; height: 16px; border-radius: 50%; cursor: pointer; border: 2px solid #8b5cf6; transition: all 0.2s; box-shadow: 0 0 8px #8b5cf6;" title="Purple"></div>
                        <div class="color-dot" onclick="setWhiteboardColor('#10b981', this)" style="background: #10b981; width: 16px; height: 16px; border-radius: 50%; cursor: pointer; border: 2px solid transparent; transition: all 0.2s;" title="Emerald"></div>
                        <div class="color-dot" onclick="setWhiteboardColor('#3b82f6', this)" style="background: #3b82f6; width: 16px; height: 16px; border-radius: 50%; cursor: pointer; border: 2px solid transparent; transition: all 0.2s;" title="Blue"></div>
                        <div class="color-dot" onclick="setWhiteboardColor('#f59e0b', this)" style="background: #f59e0b; width: 16px; height: 16px; border-radius: 50%; cursor: pointer; border: 2px solid transparent; transition: all 0.2s;" title="Amber"></div>
                        <div class="color-dot" onclick="setWhiteboardColor('#ef4444', this)" style="background: #ef4444; width: 16px; height: 16px; border-radius: 50%; cursor: pointer; border: 2px solid transparent; transition: all 0.2s;" title="Red"></div>
                        <div class="color-dot" onclick="setWhiteboardColor('#ffffff', this)" style="background: #ffffff; width: 16px; height: 16px; border-radius: 50%; cursor: pointer; border: 2px solid transparent; transition: all 0.2s;" title="White"></div>
                        <input type="color" id="whiteboardCustomColor" onchange="setWhiteboardCustomColor(this.value)" style="border: none; background: none; width: 22px; height: 22px; padding: 0; cursor: pointer; border-radius: 50%;" title="Custom Picker">
                    </div>

                    <!-- Size Slider -->
                    <div style="display: flex; gap: 0.5rem; align-items: center; background: rgba(0,0,0,0.3); border-radius: 8px; padding: 0.35rem 0.6rem; border: 1px solid var(--border-color);">
                        <span style="font-size: 0.75rem; color: var(--text-muted);"><i class="fa-solid fa-circle" style="font-size: 0.5rem;"></i> Size:</span>
                        <input type="range" id="whiteboardBrushSize" min="1" max="50" value="4" oninput="setWhiteboardBrushSize(this.value)" style="width: 70px; cursor: pointer; accent-color: var(--accent-violet);">
                        <span id="brushSizeVal" style="font-size: 0.75rem; font-family: monospace; color: var(--text-primary);">4px</span>
                    </div>

                    <!-- Actions -->
                    <button class="btn" onclick="deleteSelectedWhiteboardObjects()" title="Delete Selected Objects" style="width: auto; padding: 0.45rem 0.75rem; background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.2); color: #f87171; font-size: 0.8rem; border-radius: 6px; cursor: pointer;">
                        <i class="fa-solid fa-trash-can"></i> Clear Selection
                    </button>
                    <button class="btn" onclick="clearWhiteboard()" title="Clear Canvas" style="width: auto; padding: 0.45rem 0.75rem; background: rgba(255, 255, 255, 0.05); border: 1px solid var(--border-color); color: var(--text-primary); font-size: 0.8rem; border-radius: 6px; cursor: pointer;">
                        <i class="fa-solid fa-rotate"></i> Reset Canvas
                    </button>
                    <button class="btn btn-primary" onclick="openSaveSnapshotModal()" style="width: auto; padding: 0.45rem 1.25rem; font-size: 0.8rem; font-weight: 600; border-radius: 6px; cursor: pointer;">
                        <i class="fa-solid fa-cloud-arrow-up"></i> Save Snapshot
                    </button>
                </div>
            </div>
            
            <!-- Canvas Drawing Workspace Container -->
            <div class="glass-panel" style="padding: 0; border-radius: 12px; overflow: hidden; border: 1px solid var(--border-color); display: flex; justify-content: center; align-items: center; background: #12131a; min-height: 520px; position: relative;">
                <canvas id="whiteboard-canvas" width="900" height="500" style="border-radius: 8px; box-shadow: inset 0 0 20px rgba(0,0,0,0.8);"></canvas>
            </div>
        </div>
...
<!-- 3. Meeting Session Modal -->
<div id="sessionModal" class="modal-overlay" onclick="closeModalOnOutsideClick(event, 'sessionModal')">
    <div class="modal-container glass-panel">
        <div class="modal-header">
            <h3>Schedule Study Meeting</h3>
            <button class="modal-close" onclick="closeModal('sessionModal')">&times;</button>
        </div>
        <form action="{{ route('sessions.store', $group->id) }}" method="POST">
            @csrf
            <div class="form-group">
                <label for="sess_title">Session Title</label>
                <input type="text" name="title" id="sess_title" class="form-control" placeholder="e.g. SN1 Mechanism Peer Review" required>
            </div>
            
            <div class="form-group">
                <label for="sess_desc">Meeting Description / Goals</label>
                <textarea name="description" id="sess_desc" class="form-control" placeholder="What questions or chapters will we cover?"></textarea>
            </div>

            <div style="display: grid; grid-template-columns: 1.5fr 1fr; gap: 1rem;">
                <div class="form-group">
                    <label for="sess_date">Date & Time</label>
                    <input type="datetime-local" name="scheduled_at" id="sess_date" class="form-control" required>
                </div>
                
                <div class="form-group">
                    <label for="sess_dur">Duration (mins)</label>
                    <input type="number" name="duration_minutes" id="sess_dur" class="form-control" value="60" min="5" max="480" required>
                </div>
            </div>

            <div class="form-group">
                <label for="sess_link">Google Meet / Video Link</label>
                <input type="url" name="meeting_link" id="sess_link" class="form-control" placeholder="https://meet.google.com/xyz-uvwx-123">
            </div>
...
<!-- 4. Save Whiteboard Snapshot Modal -->
<div id="snapshotModal" class="modal-overlay" onclick="closeModalOnOutsideClick(event, 'snapshotModal')">
    <div class="modal-container glass-panel">
        <div class="modal-header">
            <h3>Save Whiteboard Snapshot</h3>
            <button class="modal-close" onclick="closeModal('snapshotModal')">&times;</button>
        </div>
        <form onsubmit="submitWhiteboardSnapshot(event)">
            @csrf
            <div class="form-group">
                <label for="snap_title">Snapshot Title</label>
                <input type="text" id="snap_title" class="form-control" placeholder="e.g. SN2 Mechanism Draw" required>
            </div>
            
            <div class="form-group">
                <label for="snap_desc">Snapshot Description</label>
                <textarea id="snap_desc" class="form-control" placeholder="Explain what this diagram is about..."></textarea>
            </div>

            <div style="margin-bottom: 1.5rem; text-align: center; background: rgba(0, 0, 0, 0.4); border-radius: 8px; padding: 0.5rem; border: 1px solid var(--border-color); overflow: hidden;">
                <span style="font-size: 0.75rem; color: var(--text-secondary); display: block; margin-bottom: 0.5rem;">Preview</span>
                <img id="snapshotPreviewImg" src="" style="max-width: 100%; max-height: 180px; border-radius: 4px; border: 1px solid var(--border-color); background: #12131a;">
            </div>

            <button type="submit" id="saveSnapshotSubmitBtn" class="btn btn-primary">Save to Drive</button>
        </form>
    </div>
</div>
...
<script>
    ...
    function switchTab(evt, tabName) {
        ...
        if (tabName === 'chat') {
            ...
            if (!monacoEditor) {
                // Initialize Monaco dynamically when selected
                initMonacoEditor();
            } else {
                // Relayout editor to avoid 10x10px rendering issues
                setTimeout(() => {
                    monacoEditor.layout();
                }, 100);
            }
...
<!-- Monaco Editor AMD Loader Script -->
<script src="https://cdnjs.cloudflare.com/ajax/libs/monaco-editor/0.39.0/min/vs/loader.min.js"></script>
<script>
    // ==============================================
    // EMBEDDED MONACO CODE EDITOR ENGINE
    // ==============================================
    let monacoEditor = null;

    // Define standard starting code structures per language
    const languageTemplates = {
        javascript: `// CollabSphere JavaScript Arena\nconsole.log("Welcome to CollabSphere Code Arena!");\n\nfunction sum(a, b) {\n    return a + b;\n}\n\nconsole.log("Sum result: " + sum(12, 18));`,
        html: `<!-- CollabSphere HTML Sandbox -->\n<div style="background: linear-gradient(135deg, #6366f1, #8b5cf6); padding: 2rem; border-radius: 12px; color: #fff; text-align: center; font-family: sans-serif;">\n    <h1 style="margin: 0; font-size: 1.75rem;">Study Portal Live Demo!</h1>\n    <p style="margin: 0.5rem 0 0 0; opacity: 0.9;">Type HTML and see it compile instantly.</p>\n</div>`,
        python: `# CollabSphere Python Sandbox\nprint("Initializing Python environment...")\n\ncourse_units = ["Data Structures", "Recursion", "Binary Trees"]\nprint(f"Studying units: {', '.join(course_units)}")\n\ndef factorial(n):\n    return 1 if n <= 1 else n * factorial(n - 1)\n    \nprint(f"Factorial of 5: {factorial(5)}")`,
        php: `<?php\n// CollabSphere PHP Session\necho "PHP 8.2 Engine running in sandbox...\\n";\n\n$members = ["Alex", "Elena", "Marcus"];\necho "Connected Students: " . implode(", ", $members) . "\\n";\n\n$group_score = 92.5;\nif ($group_score > 90) {\n    echo "Status: Excellent study engagement!\\n";\n}`,
        cpp: `// CollabSphere C++ Runtime\n#include <iostream>\n#include <vector>\nusing namespace std;\n\nint main() {\n    cout << "C++ Compiler loaded." << endl;\n    vector<int> grades = {95, 88, 92, 79};\n    int sum = 0;\n    for(int g : grades) sum += g;\n    cout << "Class average: " << (sum / grades.size()) << "%" << endl;\n    return 0;\n}`
    };
    ...
    function changeEditorLanguage() {
        ...
        const newModel = monaco.editor.createModel(languageTemplates[lang], lang);
    ...
    function runSandboxCode() {
        ...
        if (lang === 'javascript') {
            terminal.innerHTML = "CollabSphere JS Engine v1.0.0 executing...\n\n";
            let logs = [];
            ...
            terminal.innerHTML = "Rendering HTML Sandbox Live preview...\n\n";
            ...
            // Simulated execution for compiled/server-side languages
            terminal.style.color = "#38bdf8"; // Info Blue
            terminal.innerHTML = `Compiling source code in ${lang.toUpperCase()}...\n`;
            
            let progress = 0;
            const interval = setInterval(() => {
                progress += 25;
                terminal.innerHTML += `Connecting to sandbox runner... [${progress}%]\n`;
                if (progress >= 100) {
                    ...
                    if (lang === 'python') {
                        terminal.innerHTML += "Initializing Python environment...\nStudying units: Data Structures, Recursion, Binary Trees\nFactorial of 5: 120\n\n[Process completed in 0.045 seconds]";
                    } else if (lang === 'php') {
                        terminal.innerHTML += "PHP 8.2 Engine running in sandbox...\nConnected Students: Alex, Elena, Marcus\nStatus: Excellent study engagement!\n\n[Process completed in 0.012 seconds]";
                    } else if (lang === 'cpp') {
                        terminal.innerHTML += "C++ Compiler loaded.\nClass average: 88%\n\n[Process completed in 0.098 seconds]";
                    }
                }
            }, 200);
        }
...
<style>
    /* Premium style enhancements for drawing dashboard tools */
    .btn-tool {
...
<script>
    ...
    let whiteboardColor = '#8b5cf6';
    let whiteboardBrushSize = 4;
    let currentTool = 'pencil';

    function initWhiteboardCanvas() {
        whiteboardCanvas = new fabric.Canvas('whiteboard-canvas', {
            isDrawingMode: true
        });

        // Set default brush
        whiteboardCanvas.freeDrawingBrush = new fabric.PencilBrush(whiteboardCanvas);
        whiteboardCanvas.freeDrawingBrush.color = whiteboardColor;
        whiteboardCanvas.freeDrawingBrush.width = parseInt(whiteboardBrushSize, 10);

        // Customize selection handles
        ...
        whiteboardCanvas.setBackgroundColor('#12131a', whiteboardCanvas.renderAll.bind(whiteboardCanvas));
    }

    function setWhiteboardTool(tool) {
        currentTool = tool;
        
        // Reset active states in HTML toolbar
        ...
        if (tool === 'pencil') {
            ...
            whiteboardCanvas.isDrawingMode = true;
            // Draw with background color to simulate eraser
            whiteboardCanvas.freeDrawingBrush.color = '#12131a';
            whiteboardCanvas.freeDrawingBrush.width = 25; // thicker for erasing
        } else if (tool === 'select') {
            whiteboardCanvas.isDrawingMode = false;
        }
    }

    function setWhiteboardColor(color, element) {
        whiteboardColor = color;
        
        // Remove active state from color dots
        ...
        // Auto-switch to pencil tool if click color while in eraser/select mode
        if (currentTool === 'eraser' || currentTool === 'select') {
            setWhiteboardTool('pencil');
        } else if (whiteboardCanvas && whiteboardCanvas.isDrawingMode) {
            whiteboardCanvas.freeDrawingBrush.color = color;
        }
    }

    function setWhiteboardCustomColor(color) {
        setWhiteboardColor(color, null);
        document.getElementById('whiteboardCustomColor').style.boxShadow = '0 0 10px ' + color;
    }

    function setWhiteboardBrushSize(size) {
        whiteboardBrushSize = size;
        document.getElementById('brushSizeVal').innerText = size + 'px';
        
        if (whiteboardCanvas && whiteboardCanvas.isDrawingMode) {
            whiteboardCanvas.freeDrawingBrush.width = parseInt(size, 10);
        }
    }

    function addWhiteboardShape(shapeType) {
        if (!whiteboardCanvas) return;

        // Force selection mode to allow manipulating shape immediately
        setWhiteboardTool('select');

        let shape = null;
        const strokeW = parseInt(whiteboardBrushSize, 10);
        ...
        if (shapeType === 'rect') {
            ...
            shape = new fabric.IText('Study Room Note...', {
                left: 150,
                top: 150,
                fill: whiteboardColor,
        ...
        if (shape) {
            whiteboardCanvas.add(shape);
            whiteboardCanvas.setActiveObject(shape);
            whiteboardCanvas.renderAll();
        }
    }

    function deleteSelectedWhiteboardObjects() {
        if (!whiteboardCanvas) return;
        const activeObject = whiteboardCanvas.getActiveObject();
        if (activeObject) {
            whiteboardCanvas.remove(activeObject);
            whiteboardCanvas.discardActiveObject();
            whiteboardCanvas.renderAll();
        } else {
            alert("No item selected! Click Selection Mode and select an object first.");
        }
    }

    function clearWhiteboard() {
        if (!whiteboardCanvas) return;
        if (confirm("Are you sure you want to reset the whiteboard canvas? All current drawings will be lost.")) {
    ...
    function openSaveSnapshotModal() {
        ...
        // Deselect objects to avoid showing selection lines in snapshot
        whiteboardCanvas.discardActiveObject();
        whiteboardCanvas.renderAll();

        setTimeout(() => {
            const dataURL = whiteboardCanvas.toDataURL({
                format: 'png',
                quality: 1.0
            });

            document.getElementById('snapshotPreviewImg').src = dataURL;
            document.getElementById('snap_title').value = 'Whiteboard Snapshot - ' + new Date().toLocaleDateString();
            document.getElementById('snap_desc').value = 'Snapshot of workspace whiteboard created during group study.';
            
            openModal('snapshotModal');
        }, 100);
    }

    function submitWhiteboardSnapshot(e) {
        e.preventDefault();
        
        const title = document.getElementById('snap_title').value.trim();
        const desc = document.getElementById('snap_desc').value.trim();
        const image = document.getElementById('snapshotPreviewImg').src;
        
        if (!title || !image) return;

        const submitBtn = document.getElementById('saveSnapshotSubmitBtn');
        submitBtn.disabled = true;
        submitBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Saving snapshot...';

        const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');

        fetch("{{ route('resources.storeSnapshot', $group->id) }}", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                // Refresh to show newly added resource in resources drive tab!
        ...
        .catch(err => {
            ...
            alert("Error saving snapshot connection failed.");
        });
    }
